...
Rohac.prototype.expandFile = function (fileName, srcPath, destPath) {
  ...
  jsdom.env({
    file: srcPath + fileName,
      ...
      that.includeHtml($, srcPath);

      var model = new Model($('[rh-scope]'), $, srcPath);
...
/**
 * Include external rh file which is actually html
 * @param {jquery} $
 * @param {string} srcPath
 */
Rohac.prototype.includeHtml = function ($, srcPath) {
  $('[rh-html]').each(function () {
    $(this).replaceWith(fs.readFileSync(srcPath + $(this).attr('rh-html'), 'utf-8'));
  });
};
...
Rohac.prototype.generateClientAPI = function ($, model, view, destPath, prefix) {
  var rsl = 'rohac.model.scopes = ' + printObj(model.scopes) + ';' + 
            ...
            'rohac.watcher = ' + printObj(require('./watcher.js')) + ';';
